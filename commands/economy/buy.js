  ...
  const items = require("../../models/shopItems")
	...
	module.exports = {
      ...
      if(args[1] === "0") return message.channel.send(`[${client.config.error}] Bruh. You cant buy item **0x** **${args[0][0].toUpperCase()}${args[0].slice(1).toLowerCase()}**! You must atleast buy 1 item or just use XDDbuy (item) to buy that item 1x!`)
      if(args[1]) {
        if(isNaN(args[1])) return message.channel.send(`[${client.config.error}] Bruh. If you want to buy the item in a specified amount, then the amount must be a number! And not a word/letter! You can use only XDDbuy (item) to buy 1x (item)!`)
      }
      ...
      message.reply(`[${client.config.success}] You have successfully bought **${amount}x** **${itemToBuy[0].toUpperCase()}${itemToBuy.slice(1).toLowerCase()}** ${itemEmoji} for ${itemPrice} :dollar:!`)
