...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!amount) return message.channel.send(`[${client.emoji.error}] Please specify a amount of level to give`)
    if(isNaN(amount)) return message.channel.send(`[${client.emoji.error}] Please specify a valid amount of level to give`)
    ...
    client.appendLevel(member.id, message.guild.id, amountToAdd)
    
    if(member.id === message.author.id) {
      message.channel.send(`[${client.emoji.success}] Successfully added \`${args[0]}\` level to you.`)
    } else {
      message.channel.send(`[${client.emoji.success}] Successfully added \`${args[0]}\` level to \`${member.user.tag}\``)
    }
  }
}