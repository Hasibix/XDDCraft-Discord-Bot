  ...
  try {
    ...
    if(!logger) throw new SyntaxError("CHANNEL NOT FOUND")
      ...
      try{
        ...
        const hook = new Discord.WebhookClient(c.config.auditlogger.webhook.url);
        if(!hook) return createandsavewebhook();
        hook.send({
          username: c.user.username,
          avatarURL: c.displayAvatarURL({dynamic: false}),
          embeds: [LogEmbed],
        })
      }catch{
    ...
    /*
      ...
      logger.createWebhook(guild.name, {
        ...
        try{
            ...
            oldconfig.auditlogger.webhook.url = webhook.url; //webhook id
